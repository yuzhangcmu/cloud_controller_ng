...
module VCAP::CloudController
  describe TcpRouteValidator do
    let(:validator) { TcpRouteValidator.new(routing_api_client, domain_guid, route_attrs) }
    ...
    context 'when creating a route with a null port value' do
      ...
      context 'with a tcp domain' do
        ...
        it 'raises a RouteInvalid error' do
          expect { validator.validate }.
              to raise_error(TcpRouteValidator::RouteInvalid, 'Port is required, as domain belongs to a TCP router group.')
        end
      end
    end
    ...
    context 'when creating a route with a port value that is not null' do
      ...
      context 'with a domain with a router_group_guid and type tcp' do
        ...
        context 'host is included in request' do
          ...
          it 'raises a RouteInvalid error' do
            expect { validator.validate }.
                to raise_error(TcpRouteValidator::RouteInvalid, 'Host and path are not supported, as domain belongs to a TCP router group.')
          end
        end
