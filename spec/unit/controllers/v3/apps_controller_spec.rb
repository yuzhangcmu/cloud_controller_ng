        ...
        context 'lifecycle data' do
          context 'when the user is setting the buildpack' do
            ...
            it 'sets the buildpack to the user provided buildpack' do
              apps_controller.update(app_model.guid)
              expect(app_model.reload.lifecycle.data.buildpack).to eq(buildpack_url)
            end
          end
          ...
          context 'when the user does not provide a buildpack' do
            ...
            it 'resets the buildpack' do
              expect(app_model.lifecycle.buildpack).to_not be_nil
              apps_controller.update(app_model.guid)
              expect(app_model.reload.lifecycle.buildpack).to be_nil
            end
          end
          ...
          context 'when a user specifies a stack' do
            context 'when the requested stack is valid' do
              ...
              it 'sets the stack to the user provided stack' do
                apps_controller.update(app_model.guid)
                expect(app_model.reload.lifecycle.data.stack).to eq('redhat')
              end
            end
            ...
            context 'when the requested stack is invalid' do
              ...
              it 'returns an UnprocessableEntity error' do
                expect {
                  ...
                  expect(error.name).to eq 'UnprocessableEntity'
                  expect(error.response_code).to eq(422)
                  expect(error.message).to match('must exist in our DB')
                end
              end
            end

            context 'when a user wants to reset the stack' do
              let(:req_body) do
                { name: new_name,
                  lifecycle: {
                  type: 'buildpack',
              ...
              it 'resets the stack' do
                expect(app_model.lifecycle.stack).to_not be_nil
                apps_controller.update(app_model.guid)
                expect(app_model.reload.lifecycle.stack).to be_nil
              end
            end
          end

          #TODO: Check with Dieu if this is behavior we want
          context 'when a user does not provide any data' do
            ...
            it 'does not modify the lifecycle data' do
              expect(app_model.lifecycle.stack).to be 'og-stack'
              apps_controller.update(app_model.guid)
              expect(app_model.reload.lifecycle.stack).to be 'og-stack'
            end
          end
        end
      ...
      context 'lifecycle data' do
        ...
        context 'when the space developer requests lifecycle data' do
          context 'and leaves part of the data blank' do
            ...
            it 'creates the app with the lifecycle data provided' do
              response_code, response = apps_controller.update(app_model.guid)
              created_app = AppModel.last
              ...
              expect(app_lifecycle_data.stack).to eq(original_stack)
              expect(app_lifecycle_data.buildpack).to eq(nil)
            end
          end
