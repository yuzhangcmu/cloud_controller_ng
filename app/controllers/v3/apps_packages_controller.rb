...
class AppsPackagesController < ApplicationController
  ...
  def index
    ...
    paginated_result = SequelPaginator.new.get_page(app_model.packages_dataset, pagination_options)
  ...
  def create_copy
    ...
    source_package = PackageModel.where(guid: source_package_guid).eager(:app, :space, space: :organization).all.first
